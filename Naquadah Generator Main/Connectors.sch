EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 14 18
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Text Notes 8150 6400 0    79   ~ 16
PCB Mounting
$Comp
L Naquadah_Generator:MountingHole H1
U 1 1 5EDC15EC
P 8200 5500
F 0 "H1" H 8300 5546 50  0000 L CNN
F 1 "PCB Mounting Hole" H 8300 5455 50  0000 L CNN
F 2 "Naquadah_Generator:MountingHole_2.2mm_M2" H 8200 5500 50  0001 C CNN
F 3 "None" H 8200 5500 50  0001 C CNN
F 4 "None" H 8200 5500 50  0001 C CNN "Description"
F 5 "None" H 8200 5500 50  0001 C CNN "Source Part No."
F 6 "None" H 8200 5500 50  0001 C CNN "Manufacturer Name"
F 7 "None" H 8200 5500 50  0001 C CNN "Manufacturer Part No."
F 8 "None" H 8200 5500 50  0001 C CNN "Source"
	1    8200 5500
	1    0    0    -1  
$EndComp
$Comp
L Naquadah_Generator:MountingHole H5
U 1 1 5EDC15F2
P 9500 5500
F 0 "H5" H 9600 5546 50  0000 L CNN
F 1 "Top Through Hole" H 9600 5455 50  0000 L CNN
F 2 "Naquadah_Generator:MountingHole_Top_Conn_M6" H 9500 5500 50  0001 C CNN
F 3 "None" H 9500 5500 50  0001 C CNN
F 4 "None" H 9500 5500 50  0001 C CNN "Description"
F 5 "None" H 9500 5500 50  0001 C CNN "Source Part No."
F 6 "None" H 9500 5500 50  0001 C CNN "Manufacturer Name"
F 7 "None" H 9500 5500 50  0001 C CNN "Manufacturer Part No."
F 8 "None" H 9500 5500 50  0001 C CNN "Source"
	1    9500 5500
	1    0    0    -1  
$EndComp
Text Notes 9400 6400 0    79   ~ 16
Cover Mounting
$Comp
L Naquadah_Generator:MountingHole H2
U 1 1 5EDC15F9
P 8200 5700
F 0 "H2" H 8300 5746 50  0000 L CNN
F 1 "PCB Mounting Hole" H 8300 5655 50  0000 L CNN
F 2 "Naquadah_Generator:MountingHole_2.2mm_M2" H 8200 5700 50  0001 C CNN
F 3 "None" H 8200 5700 50  0001 C CNN
F 4 "None" H 8200 5700 50  0001 C CNN "Description"
F 5 "None" H 8200 5700 50  0001 C CNN "Source Part No."
F 6 "None" H 8200 5700 50  0001 C CNN "Manufacturer Name"
F 7 "None" H 8200 5700 50  0001 C CNN "Manufacturer Part No."
F 8 "None" H 8200 5700 50  0001 C CNN "Source"
	1    8200 5700
	1    0    0    -1  
$EndComp
$Comp
L Naquadah_Generator:MountingHole H3
U 1 1 5EDC15FF
P 8200 5900
F 0 "H3" H 8300 5946 50  0000 L CNN
F 1 "PCB Mounting Hole" H 8300 5855 50  0000 L CNN
F 2 "Naquadah_Generator:MountingHole_2.2mm_M2" H 8200 5900 50  0001 C CNN
F 3 "None" H 8200 5900 50  0001 C CNN
F 4 "None" H 8200 5900 50  0001 C CNN "Description"
F 5 "None" H 8200 5900 50  0001 C CNN "Source Part No."
F 6 "None" H 8200 5900 50  0001 C CNN "Manufacturer Name"
F 7 "None" H 8200 5900 50  0001 C CNN "Manufacturer Part No."
F 8 "None" H 8200 5900 50  0001 C CNN "Source"
	1    8200 5900
	1    0    0    -1  
$EndComp
$Comp
L Naquadah_Generator:MountingHole H4
U 1 1 5EDC1605
P 8200 6100
F 0 "H4" H 8300 6146 50  0000 L CNN
F 1 "PCB Mounting Hole" H 8300 6055 50  0000 L CNN
F 2 "Naquadah_Generator:MountingHole_2.2mm_M2" H 8200 6100 50  0001 C CNN
F 3 "None" H 8200 6100 50  0001 C CNN
F 4 "None" H 8200 6100 50  0001 C CNN "Description"
F 5 "None" H 8200 6100 50  0001 C CNN "Source Part No."
F 6 "None" H 8200 6100 50  0001 C CNN "Manufacturer Name"
F 7 "None" H 8200 6100 50  0001 C CNN "Manufacturer Part No."
F 8 "None" H 8200 6100 50  0001 C CNN "Source"
	1    8200 6100
	1    0    0    -1  
$EndComp
$Comp
L Naquadah_Generator:MountingHole H6
U 1 1 5EDC160B
P 9500 5700
F 0 "H6" H 9600 5746 50  0000 L CNN
F 1 "Top Through Hole" H 9600 5655 50  0000 L CNN
F 2 "Naquadah_Generator:MountingHole_Top_Conn_M6" H 9500 5700 50  0001 C CNN
F 3 "None" H 9500 5700 50  0001 C CNN
F 4 "None" H 9500 5700 50  0001 C CNN "Description"
F 5 "None" H 9500 5700 50  0001 C CNN "Source Part No."
F 6 "None" H 9500 5700 50  0001 C CNN "Manufacturer Name"
F 7 "None" H 9500 5700 50  0001 C CNN "Manufacturer Part No."
F 8 "None" H 9500 5700 50  0001 C CNN "Source"
	1    9500 5700
	1    0    0    -1  
$EndComp
$Comp
L Naquadah_Generator:MountingHole H7
U 1 1 5EDC1611
P 9500 5900
F 0 "H7" H 9600 5946 50  0000 L CNN
F 1 "Top Through Hole" H 9600 5855 50  0000 L CNN
F 2 "Naquadah_Generator:MountingHole_Top_Conn_M6" H 9500 5900 50  0001 C CNN
F 3 "None" H 9500 5900 50  0001 C CNN
F 4 "None" H 9500 5900 50  0001 C CNN "Description"
F 5 "None" H 9500 5900 50  0001 C CNN "Source Part No."
F 6 "None" H 9500 5900 50  0001 C CNN "Manufacturer Name"
F 7 "None" H 9500 5900 50  0001 C CNN "Manufacturer Part No."
F 8 "None" H 9500 5900 50  0001 C CNN "Source"
	1    9500 5900
	1    0    0    -1  
$EndComp
$Comp
L Naquadah_Generator:MountingHole H8
U 1 1 5EDC1617
P 9500 6100
F 0 "H8" H 9600 6146 50  0000 L CNN
F 1 "Top Through Hole" H 9600 6055 50  0000 L CNN
F 2 "Naquadah_Generator:MountingHole_Top_Conn_M6" H 9500 6100 50  0001 C CNN
F 3 "None" H 9500 6100 50  0001 C CNN
F 4 "None" H 9500 6100 50  0001 C CNN "Description"
F 5 "None" H 9500 6100 50  0001 C CNN "Source Part No."
F 6 "None" H 9500 6100 50  0001 C CNN "Manufacturer Name"
F 7 "None" H 9500 6100 50  0001 C CNN "Manufacturer Part No."
F 8 "None" H 9500 6100 50  0001 C CNN "Source"
	1    9500 6100
	1    0    0    -1  
$EndComp
$Comp
L Arduino_LE:Conn_ISP J?
U 1 1 5EDC6C09
P 7100 4000
AR Path="/5EDC6C09" Ref="J?"  Part="1" 
AR Path="/5EDBBDC8/5EDC6C09" Ref="J0"  Part="1" 
F 0 "J0" H 7100 4325 50  0000 C CNN
F 1 "Conn_ISP_Prog" H 7100 4234 50  0000 C CNN
F 2 "Arduino:Connector_ISP" H 7150 4000 50  0001 C CNN
F 3 "None" H 7150 4000 50  0001 C CNN
F 4 "None" H 7100 4000 50  0001 C CNN "Source Part No."
F 5 "None" H 7100 4000 50  0001 C CNN "Manufacturer Name"
F 6 "None" H 7100 4000 50  0001 C CNN "Manufacturer Part No."
F 7 "ISP connection header for pogo pins" H 7100 4000 50  0001 C CNN "Description"
F 8 "None" H 7100 4000 50  0001 C CNN "Source"
	1    7100 4000
	1    0    0    -1  
$EndComp
Wire Wire Line
	6650 4100 6750 4100
Wire Wire Line
	6650 3900 6750 3900
Wire Wire Line
	6650 4000 6750 4000
$Comp
L Frequently_Used_LE:DGND #PWR?
U 1 1 5EDC6C17
P 7850 4200
AR Path="/5EDC6C17" Ref="#PWR?"  Part="1" 
AR Path="/5EDBBDC8/5EDC6C17" Ref="#PWR0110"  Part="1" 
F 0 "#PWR0110" H 7850 3950 50  0001 C CNN
F 1 "DGND" H 7850 4050 50  0000 C CNN
F 2 "" H 7850 4200 50  0001 C CNN
F 3 "" H 7850 4200 50  0001 C CNN
	1    7850 4200
	1    0    0    -1  
$EndComp
Wire Wire Line
	7850 4100 7450 4100
Wire Wire Line
	7850 4100 7850 4200
Wire Wire Line
	7850 3900 7450 3900
Wire Wire Line
	7850 3900 7850 3800
$Comp
L Frequently_Used_LE:+5V #PWR?
U 1 1 5EDC6C21
P 7850 3800
AR Path="/5EDC6C21" Ref="#PWR?"  Part="1" 
AR Path="/5EDBBDC8/5EDC6C21" Ref="#PWR0111"  Part="1" 
F 0 "#PWR0111" H 7850 3650 50  0001 C CNN
F 1 "+5V" H 7865 3973 50  0000 C CNN
F 2 "" H 7850 3800 50  0001 C CNN
F 3 "" H 7850 3800 50  0001 C CNN
	1    7850 3800
	1    0    0    -1  
$EndComp
Text Notes 6550 4600 0    79   ~ 16
ATMega ISP Breakout
Text HLabel 6650 3900 0    50   Input ~ 0
MISO
Text HLabel 6650 4000 0    50   Input ~ 0
SCK
Text HLabel 6650 4100 0    50   Input ~ 0
nRESET
Text Notes 4100 4600 0    79   ~ 16
ATMega USB
$Sheet
S 4250 3750 550  500 
U 5FE6860B
F0 "ATMeta USB" 50
F1 "Connector USB.sch" 50
F2 "VBUS" I L 4250 3850 50 
F3 "GND" I L 4250 4150 50 
F4 "D+" I L 4250 3950 50 
F5 "D-" I L 4250 4050 50 
$EndSheet
Wire Wire Line
	4050 4050 4250 4050
Text HLabel 4050 4050 0    50   Input ~ 0
D-
Wire Wire Line
	7450 4000 7550 4000
Text HLabel 7550 4000 2    50   Input ~ 0
MOSI
Wire Wire Line
	4050 3950 4250 3950
Text HLabel 4050 3950 0    50   Input ~ 0
D+
$Comp
L Frequently_Used_LE:DGND #PWR?
U 1 1 5FE71943
P 3800 4250
AR Path="/5FE71943" Ref="#PWR?"  Part="1" 
AR Path="/5EDBBDC8/5FE71943" Ref="#PWR018"  Part="1" 
F 0 "#PWR018" H 3800 4000 50  0001 C CNN
F 1 "DGND" H 3800 4100 50  0000 C CNN
F 2 "" H 3800 4250 50  0001 C CNN
F 3 "" H 3800 4250 50  0001 C CNN
	1    3800 4250
	1    0    0    -1  
$EndComp
Wire Wire Line
	3800 4150 3800 4250
Wire Wire Line
	4250 4150 3800 4150
Wire Wire Line
	5750 2200 5400 2200
Wire Wire Line
	5750 2100 5650 2100
$Comp
L Frequently_Used_LE:DGND #PWR?
U 1 1 5EDC6BEB
P 6300 2500
AR Path="/5EDC6BEB" Ref="#PWR?"  Part="1" 
AR Path="/5EDBBDC8/5EDC6BEB" Ref="#PWR0108"  Part="1" 
F 0 "#PWR0108" H 6300 2250 50  0001 C CNN
F 1 "DGND" H 6300 2350 50  0000 C CNN
F 2 "" H 6300 2500 50  0001 C CNN
F 3 "" H 6300 2500 50  0001 C CNN
	1    6300 2500
	1    0    0    -1  
$EndComp
Wire Wire Line
	6300 2400 5350 2400
Wire Wire Line
	6300 2400 6300 2500
Wire Wire Line
	6300 2300 5350 2300
Wire Wire Line
	6300 2300 6300 2200
$Comp
L Frequently_Used_LE:+5V #PWR?
U 1 1 5EDC6BF5
P 6300 2200
AR Path="/5EDC6BF5" Ref="#PWR?"  Part="1" 
AR Path="/5EDBBDC8/5EDC6BF5" Ref="#PWR0109"  Part="1" 
F 0 "#PWR0109" H 6300 2050 50  0001 C CNN
F 1 "+5V" H 6315 2373 50  0000 C CNN
F 2 "" H 6300 2200 50  0001 C CNN
F 3 "" H 6300 2200 50  0001 C CNN
	1    6300 2200
	1    0    0    -1  
$EndComp
$Comp
L Naquadah_Generator:State_Sensing_Conn J?
U 1 1 5EDC6BFE
P 5200 2250
AR Path="/5EDC6BFE" Ref="J?"  Part="1" 
AR Path="/5EDBBDC8/5EDC6BFE" Ref="J2"  Part="1" 
F 0 "J2" H 5200 1950 50  0000 C CNN
F 1 "State_Sensing_Conn" H 5200 2550 50  0000 C CNN
F 2 "Naquadah_Generator:Daughter_Board_Hor_4_Pin_JST_Conn" H 5200 1600 50  0001 C CNN
F 3 "http://www.jst-mfg.com/product/pdf/eng/eSH.pdf" H 5200 2300 50  0001 C CNN
F 4 "455-1804-1-ND" H 5200 1900 50  0001 C CNN "Source Part No."
F 5 "JST Sales America Inc." H 5200 1800 50  0001 C CNN "Manufacturer Name"
F 6 "SM04B-SRSS-TB(LF)(SN)" H 5200 1700 50  0001 C CNN "Manufacturer Part No."
F 7 "Connector Header Surface Mount, Right Angle 4 position 0.039\" (1.00mm)" H 5200 2250 50  0001 C CNN "Description"
F 8 "Digikey" H 5200 2250 50  0001 C CNN "Source"
	1    5200 2250
	-1   0    0    1   
$EndComp
Text Notes 5150 2850 0    79   ~ 16
Sensing Board 2
Text HLabel 5750 2100 2    50   Input ~ 0
HALLEFF3
Text HLabel 5750 2200 2    50   Input ~ 0
HALLEFF4
$Comp
L Frequently_Used_LE:TestPoint_1 TP5
U 1 1 5ED9FCBB
P 5400 2000
F 0 "TP5" H 5400 2300 50  0000 C CNN
F 1 "TestPoint" H 5300 1750 50  0001 L CNN
F 2 "Frequently_Used_LE:Pad_2.5-2.5" H 5600 2000 50  0001 C CNN
F 3 "None" H 5600 2000 50  0001 C CNN
F 4 "HE1" H 5400 2200 50  0000 C CNN "Name"
F 5 "Test point" H 5400 2000 50  0001 C CNN "Description"
F 6 "None" H 5400 2000 50  0001 C CNN "Source Part No."
F 7 "None" H 5400 2000 50  0001 C CNN "Manufacturer Name"
F 8 "None" H 5400 2000 50  0001 C CNN "Manufacturer Part No."
F 9 "None" H 5400 2000 50  0001 C CNN "Source"
	1    5400 2000
	1    0    0    -1  
$EndComp
$Comp
L Frequently_Used_LE:TestPoint_1 TP6
U 1 1 5ED9FCC2
P 5650 2000
F 0 "TP6" H 5650 2300 50  0000 C CNN
F 1 "TestPoint" H 5550 1750 50  0001 L CNN
F 2 "Frequently_Used_LE:Pad_2.5-2.5" H 5850 2000 50  0001 C CNN
F 3 "None" H 5850 2000 50  0001 C CNN
F 4 "HE2" H 5650 2200 50  0000 C CNN "Name"
F 5 "Test point" H 5650 2000 50  0001 C CNN "Description"
F 6 "None" H 5650 2000 50  0001 C CNN "Source Part No."
F 7 "None" H 5650 2000 50  0001 C CNN "Manufacturer Name"
F 8 "None" H 5650 2000 50  0001 C CNN "Manufacturer Part No."
F 9 "None" H 5650 2000 50  0001 C CNN "Source"
	1    5650 2000
	1    0    0    -1  
$EndComp
Wire Wire Line
	5400 2000 5400 2200
Wire Wire Line
	5650 2000 5650 2100
Connection ~ 5400 2200
Wire Wire Line
	5400 2200 5350 2200
Connection ~ 5650 2100
Wire Wire Line
	5650 2100 5350 2100
$Comp
L Frequently_Used_LE:D D?
U 1 1 5FEFE498
P 3450 3850
AR Path="/5EA6751D/5F3CCBF3/5FEFE498" Ref="D?"  Part="1" 
AR Path="/5EDBBDC8/5FEFE498" Ref="D31"  Part="1" 
F 0 "D31" H 3350 4000 59  0000 L TNN
F 1 "5A 100V" H 3250 3750 59  0000 L CNN
F 2 "Li_Ion_LE:DO-214AC" H 3450 3850 50  0001 C CNN
F 3 "https://www.vishay.com/docs/87610/vssaf510.pdf" H 3450 3850 50  0001 C CNN
F 4 "Diode Schottky 100V 5A Surface Mount DO-221AC (SlimSMA)" H 3450 3850 50  0001 C CNN "Description"
F 5 "VSSAF510-M3/HGICT-ND" H 3450 3850 50  0001 C CNN "Source Part No."
F 6 "VSSAF510-M3/H" H 3450 3850 50  0001 C CNN "Manufacturer Part No."
F 7 "Vishay Semiconductor Diodes Division" H 3450 3850 50  0001 C CNN "Manufacturer Name"
F 8 "Digikey" H 3450 3850 50  0001 C CNN "Source"
	1    3450 3850
	1    0    0    -1  
$EndComp
Connection ~ 3700 3850
Wire Wire Line
	3700 3850 3600 3850
Wire Wire Line
	4250 3850 4100 3850
$Comp
L Frequently_Used_LE:VBUS #PWR?
U 1 1 5FF73474
P 3700 3750
AR Path="/5FF73474" Ref="#PWR?"  Part="1" 
AR Path="/5EDBBDC8/5FF73474" Ref="#PWR0112"  Part="1" 
F 0 "#PWR0112" H 3700 3600 50  0001 C CNN
F 1 "VBUS" H 3700 3900 50  0000 C CNN
F 2 "" H 3700 3750 50  0001 C CNN
F 3 "" H 3700 3750 50  0001 C CNN
	1    3700 3750
	1    0    0    -1  
$EndComp
Wire Wire Line
	3700 3850 3700 3750
$Comp
L Frequently_Used_LE:+5V #PWR?
U 1 1 5FEFFCD2
P 2800 3750
AR Path="/5FEFFCD2" Ref="#PWR?"  Part="1" 
AR Path="/5EDBBDC8/5FEFFCD2" Ref="#PWR045"  Part="1" 
F 0 "#PWR045" H 2800 3600 50  0001 C CNN
F 1 "+5V" H 2815 3923 50  0000 C CNN
F 2 "" H 2800 3750 50  0001 C CNN
F 3 "" H 2800 3750 50  0001 C CNN
	1    2800 3750
	1    0    0    -1  
$EndComp
Wire Wire Line
	2800 3850 2800 3750
Wire Wire Line
	3050 2100 2950 2100
$Comp
L Frequently_Used_LE:DGND #PWR?
U 1 1 5EDC6BCE
P 3600 2500
AR Path="/5EDC6BCE" Ref="#PWR?"  Part="1" 
AR Path="/5EDBBDC8/5EDC6BCE" Ref="#PWR0106"  Part="1" 
F 0 "#PWR0106" H 3600 2250 50  0001 C CNN
F 1 "DGND" H 3600 2350 50  0000 C CNN
F 2 "" H 3600 2500 50  0001 C CNN
F 3 "" H 3600 2500 50  0001 C CNN
	1    3600 2500
	1    0    0    -1  
$EndComp
Wire Wire Line
	3600 2400 2650 2400
Wire Wire Line
	3600 2400 3600 2500
Wire Wire Line
	3600 2300 2650 2300
Wire Wire Line
	3600 2300 3600 2200
$Comp
L Frequently_Used_LE:+5V #PWR?
U 1 1 5EDC6BD8
P 3600 2200
AR Path="/5EDC6BD8" Ref="#PWR?"  Part="1" 
AR Path="/5EDBBDC8/5EDC6BD8" Ref="#PWR0107"  Part="1" 
F 0 "#PWR0107" H 3600 2050 50  0001 C CNN
F 1 "+5V" H 3615 2373 50  0000 C CNN
F 2 "" H 3600 2200 50  0001 C CNN
F 3 "" H 3600 2200 50  0001 C CNN
	1    3600 2200
	1    0    0    -1  
$EndComp
$Comp
L Naquadah_Generator:State_Sensing_Conn J?
U 1 1 5EDC6BE1
P 2500 2250
AR Path="/5EDC6BE1" Ref="J?"  Part="1" 
AR Path="/5EDBBDC8/5EDC6BE1" Ref="J1"  Part="1" 
F 0 "J1" H 2500 1950 50  0000 C CNN
F 1 "State_Sensing_Conn" H 2500 2550 50  0000 C CNN
F 2 "Naquadah_Generator:Daughter_Board_Hor_4_Pin_JST_Conn" H 2500 1600 50  0001 C CNN
F 3 "http://www.jst-mfg.com/product/pdf/eng/eSH.pdf" H 2500 2300 50  0001 C CNN
F 4 "455-1804-1-ND" H 2500 1900 50  0001 C CNN "Source Part No."
F 5 "JST Sales America Inc." H 2500 1800 50  0001 C CNN "Manufacturer Name"
F 6 "SM04B-SRSS-TB(LF)(SN)" H 2500 1700 50  0001 C CNN "Manufacturer Part No."
F 7 "Connector Header Surface Mount, Right Angle 4 position 0.039\" (1.00mm)" H 2500 2250 50  0001 C CNN "Description"
F 8 "Digikey" H 2500 2250 50  0001 C CNN "Source"
	1    2500 2250
	-1   0    0    1   
$EndComp
Text Notes 2450 2850 0    79   ~ 16
Sensing Board 1
Text HLabel 3050 2100 2    50   Input ~ 0
HALLEFF2
Text HLabel 3050 2200 2    50   Input ~ 0
HALLEFF1
$Comp
L Frequently_Used_LE:TestPoint_1 TP3
U 1 1 5ED8A970
P 2700 2000
F 0 "TP3" H 2700 2300 50  0000 C CNN
F 1 "TestPoint" H 2600 1750 50  0001 L CNN
F 2 "Frequently_Used_LE:Pad_2.5-2.5" H 2900 2000 50  0001 C CNN
F 3 "None" H 2900 2000 50  0001 C CNN
F 4 "HE1" H 2700 2200 50  0000 C CNN "Name"
F 5 "Test point" H 2700 2000 50  0001 C CNN "Description"
F 6 "None" H 2700 2000 50  0001 C CNN "Source Part No."
F 7 "None" H 2700 2000 50  0001 C CNN "Manufacturer Name"
F 8 "None" H 2700 2000 50  0001 C CNN "Manufacturer Part No."
F 9 "None" H 2700 2000 50  0001 C CNN "Source"
	1    2700 2000
	1    0    0    -1  
$EndComp
$Comp
L Frequently_Used_LE:TestPoint_1 TP4
U 1 1 5ED8C4D5
P 2950 2000
F 0 "TP4" H 2950 2300 50  0000 C CNN
F 1 "TestPoint" H 2850 1750 50  0001 L CNN
F 2 "Frequently_Used_LE:Pad_2.5-2.5" H 3150 2000 50  0001 C CNN
F 3 "None" H 3150 2000 50  0001 C CNN
F 4 "HE2" H 2950 2200 50  0000 C CNN "Name"
F 5 "Test point" H 2950 2000 50  0001 C CNN "Description"
F 6 "None" H 2950 2000 50  0001 C CNN "Source Part No."
F 7 "None" H 2950 2000 50  0001 C CNN "Manufacturer Name"
F 8 "None" H 2950 2000 50  0001 C CNN "Manufacturer Part No."
F 9 "None" H 2950 2000 50  0001 C CNN "Source"
	1    2950 2000
	1    0    0    -1  
$EndComp
Wire Wire Line
	2700 2000 2700 2200
Connection ~ 2700 2200
Wire Wire Line
	2700 2200 2650 2200
Wire Wire Line
	3050 2200 2700 2200
Wire Wire Line
	2950 2000 2950 2100
Connection ~ 2950 2100
Wire Wire Line
	2950 2100 2650 2100
$Comp
L Frequently_Used_LE:PWR_FLAG #FLG?
U 1 1 5FF7EEF1
P 4100 3750
AR Path="/5EA6751D/5FF7EEF1" Ref="#FLG?"  Part="1" 
AR Path="/5EDBBDC8/5FF7EEF1" Ref="#FLG0105"  Part="1" 
F 0 "#FLG0105" H 4100 3825 50  0001 C CNN
F 1 "PWR_FLAG" H 4100 3923 50  0000 C CNN
F 2 "" H 4100 3750 50  0001 C CNN
F 3 "~" H 4100 3750 50  0001 C CNN
	1    4100 3750
	1    0    0    -1  
$EndComp
Wire Wire Line
	4100 3850 4100 3750
Connection ~ 4100 3850
Wire Wire Line
	4100 3850 3700 3850
$Comp
L Jumper:SolderJumper_2_Bridged JP?
U 1 1 5FF89968
P 3050 3850
AR Path="/5F06C1CA/5FF89968" Ref="JP?"  Part="1" 
AR Path="/5EDBBDC8/5FF89968" Ref="JP4"  Part="1" 
F 0 "JP4" V 3004 3918 50  0000 L CNN
F 1 "Bridged" V 3095 3918 50  0000 L CNN
F 2 "Jumper_LE:SolderJumper-2_P1.3mm_Bridged_RoundedPad1.0x1.5mm" H 3050 3850 50  0001 C CNN
F 3 "None" H 3050 3850 50  0001 C CNN
F 4 "None" H 3050 3850 50  0001 C CNN "Description"
F 5 "None" H 3050 3850 50  0001 C CNN "Manufacturer Name"
F 6 "None" H 3050 3850 50  0001 C CNN "Manufacturer Part No."
F 7 "None" H 3050 3850 50  0001 C CNN "Name"
F 8 "None" H 3050 3850 50  0001 C CNN "Source"
F 9 "None" H 3050 3850 50  0001 C CNN "Source Part No."
	1    3050 3850
	1    0    0    -1  
$EndComp
Wire Wire Line
	3300 3850 3200 3850
Wire Wire Line
	2900 3850 2800 3850
Text HLabel 9050 2100 2    50   Input ~ 0
BATT-
Text HLabel 9050 2000 2    50   Input ~ 0
BATT+
Text Label 8850 2100 2    50   ~ 0
BATT-
Text Label 8850 2000 2    50   ~ 0
BATT+
Wire Wire Line
	8550 2200 8550 2000
Wire Wire Line
	8200 2200 8200 1900
Wire Wire Line
	8200 1500 8200 1600
$Comp
L Frequently_Used_LE:TestPoint_1 TP?
U 1 1 60A6C239
P 8200 1500
AR Path="/60A6C239" Ref="TP?"  Part="1" 
AR Path="/5EA6751D/60A6C239" Ref="TP?"  Part="1" 
AR Path="/5EDBBDC8/60A6C239" Ref="TP7"  Part="1" 
F 0 "TP7" H 8200 1750 50  0000 C CNN
F 1 "TestPoint" H 8200 1220 50  0001 C CNN
F 2 "Frequently_Used_LE:Pad_2.5-2.5" H 8400 1500 50  0001 C CNN
F 3 "None" H 8400 1500 50  0001 C CNN
F 4 "SWITCH" H 8200 1650 50  0000 C CNN "Name"
F 5 "None" H 8195 1145 50  0001 C CNN "Source"
F 6 "None" H 8195 1145 50  0001 C CNN "Source Part No."
F 7 "None" H 8195 1145 50  0001 C CNN "Manufacturer Name"
F 8 "None" H 8195 1145 50  0001 C CNN "Manufacturer Part No."
F 9 "Test point" H 8200 1500 50  0001 C CNN "Description"
	1    8200 1500
	1    0    0    -1  
$EndComp
$Comp
L Frequently_Used_LE:TestPoint_1 TP?
U 1 1 60A6C245
P 8550 2200
AR Path="/60A6C245" Ref="TP?"  Part="1" 
AR Path="/5EA6751D/60A6C245" Ref="TP?"  Part="1" 
AR Path="/5EDBBDC8/60A6C245" Ref="TP9"  Part="1" 
F 0 "TP9" H 8550 2400 50  0000 C CNN
F 1 "TestPoint" H 8550 1920 50  0001 C CNN
F 2 "Frequently_Used_LE:Pad_2.5-2.5" H 8750 2200 50  0001 C CNN
F 3 "None" H 8750 2200 50  0001 C CNN
F 4 "BATT+" H 8550 2500 50  0000 C CNN "Name"
F 5 "None" H 8545 1845 50  0001 C CNN "Source"
F 6 "None" H 8545 1845 50  0001 C CNN "Source Part No."
F 7 "None" H 8545 1845 50  0001 C CNN "Manufacturer Name"
F 8 "None" H 8545 1845 50  0001 C CNN "Manufacturer Part No."
F 9 "Test point" H 8550 2200 50  0001 C CNN "Description"
	1    8550 2200
	-1   0    0    1   
$EndComp
$Comp
L Frequently_Used_LE:TestPoint_1 TP?
U 1 1 60A6C251
P 8200 2200
AR Path="/60A6C251" Ref="TP?"  Part="1" 
AR Path="/5EA6751D/60A6C251" Ref="TP?"  Part="1" 
AR Path="/5EDBBDC8/60A6C251" Ref="TP20"  Part="1" 
F 0 "TP20" H 8200 2400 50  0000 C CNN
F 1 "TestPoint" H 8200 1920 50  0001 C CNN
F 2 "Frequently_Used_LE:Pad_2.5-2.5" H 8400 2200 50  0001 C CNN
F 3 "None" H 8400 2200 50  0001 C CNN
F 4 "VOLUME" H 8200 2500 50  0000 C CNN "Name"
F 5 "None" H 8195 1845 50  0001 C CNN "Source"
F 6 "None" H 8195 1845 50  0001 C CNN "Source Part No."
F 7 "None" H 8195 1845 50  0001 C CNN "Manufacturer Name"
F 8 "None" H 8195 1845 50  0001 C CNN "Manufacturer Part No."
F 9 "Test point" H 8200 2200 50  0001 C CNN "Description"
	1    8200 2200
	-1   0    0    1   
$EndComp
Text HLabel 9050 1800 2    50   Input ~ 0
MODE
Text HLabel 9050 1700 2    50   Input ~ 0
READY
Connection ~ 8200 1600
Wire Wire Line
	8150 1600 8200 1600
Wire Wire Line
	8900 2200 8900 2100
$Comp
L Frequently_Used_LE:TestPoint_1 TP?
U 1 1 60A6C283
P 8900 2200
AR Path="/60A6C283" Ref="TP?"  Part="1" 
AR Path="/5EA6751D/60A6C283" Ref="TP?"  Part="1" 
AR Path="/5EDBBDC8/60A6C283" Ref="TP2"  Part="1" 
F 0 "TP2" H 8900 2400 50  0000 C CNN
F 1 "TestPoint" H 8800 1950 50  0001 L CNN
F 2 "Frequently_Used_LE:Pad_2.5-2.5" H 9100 2200 50  0001 C CNN
F 3 "None" H 9100 2200 50  0001 C CNN
F 4 "BATT-" H 8900 2500 50  0000 C CNN "Name"
F 5 "Test point" H 8900 2200 50  0001 C CNN "Description"
F 6 "None" H 8900 2200 50  0001 C CNN "Source Part No."
F 7 "None" H 8900 2200 50  0001 C CNN "Manufacturer Name"
F 8 "None" H 8900 2200 50  0001 C CNN "Manufacturer Part No."
F 9 "None" H 8900 2200 50  0001 C CNN "Source"
	1    8900 2200
	-1   0    0    1   
$EndComp
Text Notes 7550 2850 0    79   ~ 16
User Interface Board Connector
Text HLabel 9050 1600 2    39   Input ~ 0
SWITCH
$Comp
L Naquadah_Generator:User_Interface_6_Pin_Conn J10
U 1 1 60BE3DA9
P 8000 1850
F 0 "J10" H 8000 1500 50  0000 C CNN
F 1 "User_Interface_6_Pin_Conn" V 8450 1850 50  0000 C CNN
F 2 "Naquadah_Generator:Daughter_Board_Vert_6_Pin_JST_Conn" H 8050 1050 50  0001 C CNN
F 3 "http://www.jst-mfg.com/product/pdf/eng/eSH.pdf" H 8000 1800 50  0001 C CNN
F 4 "Connector Header Surface Mount 6 position 0.039\" (1.00mm)" H 8050 950 50  0001 C CNN "Description"
F 5 "Digikey" H 8000 850 50  0001 C CNN "Source"
F 6 "455-1792-1-ND" H 8050 1350 50  0001 C CNN "Source Part No."
F 7 "JST Sales America Inc." H 8050 1250 50  0001 C CNN "Manufacturer Name"
F 8 "BM06B-SRSS-TB(LF)(SN)" H 8050 1150 50  0001 C CNN "Manufacturer Part No."
	1    8000 1850
	-1   0    0    1   
$EndComp
Wire Wire Line
	8150 1900 8200 1900
Text HLabel 9050 1900 2    50   Input ~ 0
VOLUME
Connection ~ 8200 1900
Wire Wire Line
	8200 1900 9050 1900
Wire Wire Line
	8150 2100 8900 2100
Wire Wire Line
	8150 1800 8900 1800
Wire Wire Line
	8150 1700 8550 1700
Wire Wire Line
	8900 1500 8900 1800
$Comp
L Frequently_Used_LE:TestPoint_1 TP?
U 1 1 60BF320A
P 8900 1500
AR Path="/60BF320A" Ref="TP?"  Part="1" 
AR Path="/5EA6751D/60BF320A" Ref="TP?"  Part="1" 
AR Path="/5EDBBDC8/60BF320A" Ref="TP10"  Part="1" 
F 0 "TP10" H 8900 1750 50  0000 C CNN
F 1 "TestPoint" H 8900 1220 50  0001 C CNN
F 2 "Frequently_Used_LE:Pad_2.5-2.5" H 9100 1500 50  0001 C CNN
F 3 "None" H 9100 1500 50  0001 C CNN
F 4 "MODE" H 8900 1650 50  0000 C CNN "Name"
F 5 "None" H 8895 1145 50  0001 C CNN "Source"
F 6 "None" H 8895 1145 50  0001 C CNN "Source Part No."
F 7 "None" H 8895 1145 50  0001 C CNN "Manufacturer Name"
F 8 "None" H 8895 1145 50  0001 C CNN "Manufacturer Part No."
F 9 "Test point" H 8900 1500 50  0001 C CNN "Description"
	1    8900 1500
	1    0    0    -1  
$EndComp
Wire Wire Line
	8200 1600 9050 1600
$Comp
L Frequently_Used_LE:TestPoint_1 TP?
U 1 1 60A6C25D
P 8550 1500
AR Path="/60A6C25D" Ref="TP?"  Part="1" 
AR Path="/5EA6751D/60A6C25D" Ref="TP?"  Part="1" 
AR Path="/5EDBBDC8/60A6C25D" Ref="TP8"  Part="1" 
F 0 "TP8" H 8550 1750 50  0000 C CNN
F 1 "TestPoint" H 8550 1220 50  0001 C CNN
F 2 "Frequently_Used_LE:Pad_2.5-2.5" H 8750 1500 50  0001 C CNN
F 3 "None" H 8750 1500 50  0001 C CNN
F 4 "READY" H 8550 1650 50  0000 C CNN "Name"
F 5 "None" H 8545 1145 50  0001 C CNN "Source"
F 6 "None" H 8545 1145 50  0001 C CNN "Source Part No."
F 7 "None" H 8545 1145 50  0001 C CNN "Manufacturer Name"
F 8 "None" H 8545 1145 50  0001 C CNN "Manufacturer Part No."
F 9 "Test point" H 8550 1500 50  0001 C CNN "Description"
	1    8550 1500
	1    0    0    -1  
$EndComp
Wire Wire Line
	8550 1500 8550 1700
Connection ~ 8550 1700
Connection ~ 8900 1800
Wire Wire Line
	8550 1700 9050 1700
Wire Wire Line
	8900 1800 9050 1800
Connection ~ 8900 2100
Wire Wire Line
	8900 2100 9050 2100
Connection ~ 8550 2000
Wire Wire Line
	8550 2000 9050 2000
Wire Wire Line
	8150 2000 8550 2000
$Comp
L Connector_Generic:Conn_01x02 J?
U 1 1 5F6C6D5E
P 9400 4100
AR Path="/5EA45E37/5F6C6D5E" Ref="J?"  Part="1" 
AR Path="/5EA4676F/5F6C6D5E" Ref="J?"  Part="1" 
AR Path="/5EA46A2B/5F6C6D5E" Ref="J?"  Part="1" 
AR Path="/5ED17BA0/5ED2A32C/5F6C6D5E" Ref="J?"  Part="1" 
AR Path="/5ED17BA0/5F6C6D5E" Ref="J?"  Part="1" 
AR Path="/5EDBBDC8/5F6C6D5E" Ref="J3"  Part="1" 
F 0 "J3" H 9950 4000 50  0000 R CNN
F 1 "Conn_01x02" H 9950 4100 50  0000 R CNN
F 2 "Connector_LE:JST_SH_BM02B-SRSS-TB_1x02-1MP_P1.00mm_Vertical" H 9400 4100 50  0001 C CNN
F 3 "http://www.jst-mfg.com/product/pdf/eng/eSH.pdf" H 9400 4100 50  0001 C CNN
F 4 "Connector Header Surface Mount 2 position 0.039\" (1.00mm)" H 9400 4100 50  0001 C CNN "Description"
F 5 "JST Sales America Inc." H 9400 4100 50  0001 C CNN "Manufacturer Name"
F 6 "BM02B-SRSS-TB(LF)(SN)" H 9400 4100 50  0001 C CNN "Manufacturer Part No."
F 7 "Digikey" H 9400 4100 50  0001 C CNN "Source"
F 8 "455-1788-1-ND" H 9400 4100 50  0001 C CNN "Source Part No."
	1    9400 4100
	-1   0    0    1   
$EndComp
Wire Wire Line
	9950 3900 9950 4000
$Comp
L power:+5V #PWR?
U 1 1 5F6C6D65
P 9950 3900
AR Path="/5ED17BA0/5ED2A32C/5F6C6D65" Ref="#PWR?"  Part="1" 
AR Path="/5ED17BA0/5F6C6D65" Ref="#PWR?"  Part="1" 
AR Path="/5EDBBDC8/5F6C6D65" Ref="#PWR0102"  Part="1" 
F 0 "#PWR0102" H 9950 3750 50  0001 C CNN
F 1 "+5V" H 9965 4073 50  0000 C CNN
F 2 "" H 9950 3900 50  0001 C CNN
F 3 "" H 9950 3900 50  0001 C CNN
	1    9950 3900
	1    0    0    -1  
$EndComp
Wire Wire Line
	9950 4100 9600 4100
Wire Wire Line
	9600 4000 9950 4000
Text HLabel 9950 4100 2    50   Input ~ 0
WHITELT
Text Notes 9250 4600 0    79   ~ 16
White Lights
$EndSCHEMATC
